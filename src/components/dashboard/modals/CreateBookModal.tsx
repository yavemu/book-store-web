...
import { Genre, PublishingHouse } from '@/types/domain';
import { createBookSchema, CreateBookFormData } from '@/services/validation';
...
export function CreateBookModal({ isOpen, onClose, onSuccess }: CreateBookModalProps) {
  ...
  const validateForm = (): boolean => {
    const validationData: CreateBookFormData = {
    ...
    const validation = validateWithZodSafe(createBookSchema, validationData);
    
    if (!validation.success) {
      const newErrors: Record<string, string> = {};
      Object.entries(validation.errors).forEach(([field, messages]) => {
        newErrors[field] = messages[0]; // Primera mensaje de error
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await bookCatalogApi.create(apiData);
